...
def convertCloudFromOpen3dToRos(open3d_cloud, seq, frame_id="odom"):
    # Set "header"
    header = Header()
...
def convertCloudFromRosToOpen3d(ros_cloud):
    ...
    if "rgb" in field_names:
        ...
        # Get xyz
        xyz = [(x,y,z) for x,y,z,rgb,normal_x,normal_y,normal_z,curvature in cloud_data ] # (why cannot put this line below rgb?)

        # Get rgb
        # Check whether int or float
        if type(cloud_data[0][IDX_RGB_IN_FIELD])==float: # if float (from pcl::toROSMsg)
            rgb = [convert_rgbFloat_to_tuple(rgb) for x,y,z,rgb,normal_x,normal_y,normal_z,curvature in cloud_data ]
        else:
            rgb = [convert_rgbUint32_to_tuple(rgb) for x,y,z,rgb,normal_x,normal_y,normal_z,curvature in cloud_data ]
